...
namespace Mini_Fitness_tracker
{
    public class WorkOutPlan
    {
        ...
        //************************************************************************************************
        public static void Add_exercises(int index, List<User> users, List<Exercise> exercises)  // adding exercises to the workout plan of a user
        {
            WorkOutPlan workOutPlan = new WorkOutPlan();
            Console.Write("Enter the number of the exercise you want to add: ");
            string test = Console.ReadLine();
            int num;
            while (!int.TryParse(test, out num) || num <= 0 || num > exercises.Count)
            {
                Console.WriteLine("This value is invalid.");
                Console.Write("Please enter another value from 1 to " + exercises.Count + " : ");
                test = Console.ReadLine();
            }
            workOutPlan.Name_exercise = exercises[num - 1].NameOfExercise;
            Console.Write("Enter the time of the exercise: ");
            test = Console.ReadLine();
            while (!double.TryParse(test, out workOutPlan.time) || workOutPlan.time <= 0)
            {
                Console.WriteLine("This value is invalid.");
            ...
            workOutPlan.calories = workOutPlan.time * exercises[num - 1].CaloriesBurnedPerMin;
            ...
            for (int i = 0; i < users[index].days.Count; i++)
            {
                if (users[index].days[i].date == workOutPlan.date)
                {
                    ...
                    users[index].days[i].calories += workOutPlan.time * exercises[num - 1].CaloriesBurnedPerMin;
                    existing = true;
                    break;
                }
            }
            if (!existing)
            {
                ...
                day.calories = workOutPlan.time * exercises[num - 1].CaloriesBurnedPerMin;
        ...
        //******************************************************************************************************
        public static void Remove_exercises(int index, List<User> users, List<Exercise> exercises)      // removing exercises from the workout plan of a user
        {
            ...
            int i;
            for ( i = 0; i < users[index].days.Count; i++)
            {
            ...
            users[index].workOutPlans.RemoveAt(i);
        ...
        {
            string pathPlans = @"C:\Users\PC\source\repos\Mini_Fitness_tracker\bin\Debug\workoutplan.txt";
        ...
        public static void LoadWorkOutPlans(List<User> users)
        {
            ...
            foreach (var line in lines)
            {
                ...
                string[] plans = parts[1].Split(',', StringSplitOptions.RemoveEmptyEntries);
